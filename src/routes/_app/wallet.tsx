...
import {
  ...
  ArrowUpRight, ArrowDownLeft, Gift as GiftIcon, X,
} from "lucide-react";
...
export const Route = createFileRoute("/_app/wallet")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Premium wallet with balance, top-ups, and gift history." },
    ],
  }),
});
...
function WalletPage() {
  ...
  const stats = useMemo(() => {
    ...
    const received = tx.filter(x => x.type === "received").reduce((s, x) => s + x.amount, 0);
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      >
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button
            ...
            className="press-glow flex items-center justify-center gap-2 rounded-2xl py-3 text-sm font-bold"
            style={{
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        <StatCard label="Spent on gifts" value={stats.sent} icon={<ArrowUpRight className="h-4 w-4" />} negative />
      ...
      <div className="mt-5 flex-1">
        ...
        {tx.length === 0 ? (
          <div className="glass-card grid place-items-center p-10 text-center" style={{ borderRadius: 22 }}>
    ...
    >
      <span
        ...
        style={{
          ...
          color: isGift ? (t.giftColor ?? tint) : tint,
          boxShadow: `0 0 12px color-mix(in oklab, ${isGift ? (t.giftColor ?? tint) : tint} 50%, transparent)`,
        }}
      >
        {isGift ? <span style={{ filter: `drop-shadow(0 0 6px ${t.giftColor ?? tint})` }}>{t.giftEmoji}</span>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-soft text-[11px]">{date} · {time}</p>
      </div>
      <span
...
function AmountModal({
  title, actionLabel, presets, max, onClose, onConfirm,
}: {
  title: string;
  actionLabel: string;
  presets: number[];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="text-neon-title mb-3 text-lg">{title}</h3>
