...
function Appointments() {
  ...
  return (
    <div className="flex min-h-full flex-col animate-[fade-in_0.4s_ease-out] pb-24">
      ...
      <Section title="Today" items={todays} expanded={expanded} setExpanded={setExpanded} onToggle={toggle} onDelete={remove} onEdit={(a) => { setEditing(a); setOpen(true); }} />
      <Section title="Upcoming" items={upcoming} expanded={expanded} setExpanded={setExpanded} onToggle={toggle} onDelete={remove} onEdit={(a) => { setEditing(a); setOpen(true); }} />
      {past.length > 0 && (
        <Section title="Past" items={past} expanded={expanded} setExpanded={setExpanded} onToggle={toggle} onDelete={remove} onEdit={(a) => { setEditing(a); setOpen(true); }} />
      )}
      {items.length === 0 && (
        <p className="mt-12 text-center text-sm text-muted-foreground">No appointments yet</p>
      )}
...
function Section({
  title, items, expanded, setExpanded, onToggle, onDelete, onEdit,
}: {
  title: string;
  items: Appt[];
...
function ApptModal({ initial, onClose, onSave }: { initial: Appt | null; onClose: () => void; onSave: (a: Appt) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm animate-[fade-in_0.2s_ease-out]" onClick={onClose}>
      ...
      >
        ...
        <input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Location (optional)" className="mb-4 w-full rounded-xl border border-border bg-card/40 px-3 py-2 text-sm outline-none" />
